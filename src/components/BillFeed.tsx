...
import { fetchBills, fetchCongressYears, fetchTrendingBills, semanticSearchBillsByText, checkServerCache } from "@/lib/api";
...
import { UIBill, SemanticSearchResult } from '@/types/bills';
...
import SemanticBillGraph from "./SemanticBillGraph";
...
// Define a type for the intermediate similarity data
interface BillSimilarityInfo {
  id: string;
  similarity?: number;
}
...
const formatBillType = (type: string | null | undefined) => {
  if (!type) return ''; // Handle null or undefined
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-[11px] bg-zinc-900/30 rounded-lg p-2">
        <span className="text-zinc-300">{bill.bill_type ? `${bill.bill_type}${bill.bill_number}` : `Bill #${bill.bill_number}`}</span>
    ...
    >
      ...
      <div className="flex flex-col gap-2 mb-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Badge className="px-2 py-0.5 rounded-full font-medium bg-zinc-800/50 text-zinc-200 border border-zinc-700 text-xs sm:text-sm whitespace-nowrap">
            {bill.bill_type ? `${bill.bill_type}${bill.bill_number}` : `Bill #${bill.bill_number}`}
          </Badge>
        </div>
        {/* Engagement Score */}
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs mb-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-zinc-300">
            {bill.bill_type ? `${formatBillType(bill.bill_type)} ${bill.bill_number}` : `Bill #${bill.bill_number}`}
          </span>
        </div>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-zinc-300">
            {bill.bill_type ? `${bill.bill_type}${bill.bill_number}` : `Bill #${bill.bill_number}`} • Congress {bill.congress}
          </span>
        </div>
...
const ProcessedBillCardDesign9: React.FC<{
  ...
  // For debugging - log similarity
  if (bill.similarity !== undefined) {
    console.log(`💯 Bill ${bill.id} has similarity score: ${bill.similarity}`);
  }
    ...
    >
      ...
      <div className="flex flex-col gap-2 max-w-full">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <Badge className="px-2 py-0.5 rounded-full text-[10px] bg-zinc-800/50 text-zinc-300 border border-zinc-700">
            <span className="hidden xs:inline">{bill.cosponsors_count || 0} Sponsors</span>
            <span className="xs:hidden">{bill.cosponsors_count || 0} Sp.</span>
          </Badge>
          <Badge className="px-2 py-0.5 rounded-full text-[10px] bg-zinc-800/50 text-zinc-300 border border-zinc-700">
            <span className="hidden xs:inline">{bill.actions_count || 0} Actions</span>
            <span className="xs:hidden">{bill.actions_count || 0} Act.</span>
          </Badge>
        </div>
        
        {/* Title */}
        <h3 className="text-sm sm:text-base font-medium text-zinc-200 line-clamp-2">{bill.title}</h3>
          ...
          <div className="flex-1 min-w-[100px] max-w-[200px]">
            <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: `${getStatusProgress(bill.status)}%` }}
              />
            </div>
            <div className="flex justify-between mt-1 text-[8px] text-zinc-500">
              {stages.map((stage, index) => (
                <span 
                  key={stage} 
                  className={index <= currentStageIndex ? "text-primary-light" : ""}
                >
                  {index === 0 || index === stages.length - 1 || index === Math.floor(stages.length / 2) ? stage : "•"}
                </span>
              ))}
            </div>
          </div>
...
const ProcessedBillCard: React.FC<{
  bill: UIBill;
  onClick: () => void;
}> = ({ bill, onClick }) => {
  return <ProcessedBillCardDesign9 bill={bill} onClick={onClick} />;
};
...
const BillFeed: React.FC<BillFeedProps> = ({
  ...
  const [cachedResultsUsed, setCachedResultsUsed] = useState(false);
  // Force re-render when bills or loading state changes
  const [renderKey, setRenderKey] = useState(0);
  
  // Add a ref to track semantic search status
  const semanticSearchInProgressRef = useRef(false);
  
  // Add semantic search cache to prevent redundant API calls
  const semanticSearchCacheRef = useRef<{
    [key: string]: { 
      bills: Bill[], 
      timestamp: number,
      threshold: number
  ...
  useEffect(() => {
    // Use a ref to track if we're already loading trending bills
    ...
    const loadTrendingBills = async () => {
      ...
      try {
        // Set a timeout to show error if trending bills take too long
        console.log('🔄 Starting trending bills fetch...');
        timeoutId = setTimeout(() => {
          // Check if we still don't have trending bills after timeout
          if (trendingBills.length === 0) {
            console.warn('Trending bills fetch timed out after 30 seconds');
            setTrendingError(new Error('Trending bills loading timed out. Please refresh.'));
          }
        ...
        // Clear timeout since we got data successfully
        if (timeoutId) {
          clearTimeout(timeoutId);
          timeoutId = null;
        }
        
        console.log(`✅ Trending bills fetch completed with ${trending?.length || 0} bills`);
        
        // Only update state if we actually got some trending bills
        if (trending && trending.length > 0) {
          setTrendingBills(trending);
          setTrendingError(null);
        } else {
          // If no data, show a more helpful error
          console.warn('No trending bills returned from API');
          setTrendingError(new Error('No trending bills available'));
        }
      } catch (err) {
    ...
    // Execute the function to load trending bills
    loadTrendingBills();
    
    // Clean up function to clear timeout if component unmounts during loading
    return () => {
      if (timeoutId) {
        clearTimeout(timeoutId);
      }
  ...
  useEffect(() => {
    // Check cache status to verify server-side caching is working
    const cacheStatus = checkServerCache();
    console.log('📋 Initial Server Cache Status:', cacheStatus);
    
    // Ensure we start with semantic search disabled
  ...
  useEffect(() => {
    // Filters have changed, clear the semantic search cache
    console.log('🧹 Clearing semantic search cache due to filter changes');
    semanticSearchCacheRef.current = {};
  }, [filters]);
  ...
  const performSemanticSearch = async (query: string, threshold: number) => {
    if (!query.trim()) {
      // For empty queries, use "latest bills" as the default
      query = "latest bills";
    }
    ...
    try {
      ...
      // Create a cache key based on query and threshold
      const cacheKey = `${query.trim().toLowerCase()}_${threshold}`;
      
      // Check cache first
      const cachedResult = semanticSearchCacheRef.current[cacheKey];
      const now = Date.now();
      
      // Use cached results if they exist and aren't expired
      if (cachedResult && (now - cachedResult.timestamp < CACHE_EXPIRATION)) {
        console.log(`🔍 Using cached search results for "${query}" (threshold: ${threshold})`);
        
        // Update UI with cached results
        setBills(cachedResult.bills);
        setTotalBills(cachedResult.bills.length);
        ...
        // Set a flag to indicate that cached results were used
        // We store it in a state so we can show a UI indicator
        setCachedResultsUsed(true);
      ...
      setSemanticQuery(query);
      setSemanticThreshold(threshold);
      setCachedResultsUsed(false);
      
      // Call the API to get semantic search results
      ...
      console.log('🔍 Received semantic search results:', semanticResults?.length || 0);
      
      // Validate results
      if (!semanticResults || !Array.isArray(semanticResults) || semanticResults.length === 0) {
        console.log('🔍 No results found or invalid response format');
        setBills([]);
        setTotalBills(0); // Explicitly set to 0
        setHasMore(false);
        setPage(1);
        setLoading(false);
        // Force re-render
      ...
      // Extract bill IDs and similarity scores from semantic results
      const billsWithSimilarity: BillSimilarityInfo[] = semanticResults.map(bill => ({
        id: bill.id,
        similarity: 'similarity' in bill && typeof bill.similarity === 'number' ? bill.similarity : undefined
      })).filter(item => item.id && item.similarity !== undefined);
      
      console.log(`🔍 Found ${billsWithSimilarity.length} valid bills with similarity scores`);
      
      if (billsWithSimilarity.length === 0) {
        setBills([]);
        setTotalBills(0); // Explicitly set to 0
        setHasMore(false);
        setPage(1);
      ...
      // Get the bill IDs to fetch complete data
      const billIds = billsWithSimilarity.map(b => b.id);
      
      // Fetch complete bill data from the database
      // We'll use the regular fetchBills API but with specific IDs
      const { bills: completeBills } = await fetchBills({
        ...
        // Add specific bill IDs to fetch
        billIds: billIds
      });
      
      console.log(`🔍 Fetched ${completeBills.length} complete bills from database`);
      
      // Merge the complete bill data with similarity scores
      const processedResults = completeBills.map(bill => {
        // Find the corresponding similarity score
        const matchingResult = billsWithSimilarity.find(b => b.id === bill.id);
        
        // Create a new object with the bill data and similarity score
        return {
          ...bill,
          // Add the similarity score from semantic search
      ...
      console.log(`🔍 Processed ${processedResults.length} bills with complete data`);
      
      // Update UI immediately
      setBills(processedResults);
      // Explicitly set totalBills to match the number of results
      ...
      // After successfully getting results, update the cache
      semanticSearchCacheRef.current[cacheKey] = {
        bills: processedResults,
        timestamp: Date.now(),
        threshold
      };
      ...
      // Return a rejected promise so we can use .finally()
  ...
  const handleSearch = (term: string) => {
    // Always ensure semantic search is turned off
    setIsSemanticSearch(false);
    
    // Clear any semantic search query 
    setSemanticQuery("");
    
    // Trigger the regular search
    onSearch(term);
  };

  const handleSemanticSearch = (query: string, threshold: number) => {
    // Prevent duplicate searches for the same query and threshold
    if (
      semanticSearchInProgressRef.current || 
      (isSemanticSearch && query === semanticQuery && threshold === semanticThreshold)
    ) {
      console.log('🔍 Skipping duplicate semantic search:', query);
      return;
    }
    ...
    performSemanticSearch(query, threshold)
      .finally(() => {
        // Reset the flag when search completes
        semanticSearchInProgressRef.current = false;
      });
  };

  // Update the displayed total bills count for semantic search
  const displayedTotalBills = isSemanticSearch ? bills.length : totalBills;

  useEffect(() => {
    // Increment render key to force re-render when bills or loading state changes
    setRenderKey(prev => prev + 1);
  }, [bills, loading]);
    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex-1 p-2 sm:p-4 md:p-6">
        <div className="max-w-[1200px] mx-auto">
          {/* Bill Statistics Section - Moved to the top */}
          <BillStats />
          ...
          <div className="glass-panel p-3 sm:p-6 mb-4 sm:mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-2">
              ...
              <div className="flex items-center space-x-2 text-xs text-zinc-400 overflow-x-auto pb-1 scrollbar-hide">
                <span className="text-zinc-200 cursor-help whitespace-nowrap">Cosponsors</span>
                <span>•</span>
                <span className="text-zinc-200 cursor-help whitespace-nowrap">Actions</span>
                <span>•</span>
                <span className="text-zinc-200 cursor-help whitespace-nowrap">Status</span>
              </div>
            </div>
          ...
          <div className="glass-panel p-3 sm:p-6 mb-4 sm:mb-8">
            <SearchBar
              ...
              isLoading={loading && isSemanticSearch}
            />
          </div>
          ...
          {isSemanticSearch && semanticQuery && (
            <div className="glass-panel p-3 sm:p-6 mb-4 sm:mb-8">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  {loading ? (
                    <Loader2 className="h-5 w-5 text-primary-light animate-spin flex-shrink-0" />
                  ) : (
                    <Bot className="h-5 w-5 text-primary-light flex-shrink-0" />
                  )}
                  <h3 className="text-base sm:text-lg font-medium text-white truncate">
                    {loading ? "AI Search in Progress..." : "Semantic Search Mode"}
                  </h3>
                  {cachedResultsUsed && !loading && (
                    <span className="text-xs px-2 py-0.5 bg-emerald-500/20 text-emerald-400 rounded-full border border-emerald-500/30 whitespace-nowrap">
                      Cached ⚡
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="flex items-center gap-1 whitespace-nowrap">
                    <span className="text-xs text-zinc-400">Threshold: {semanticThreshold * 100}%</span>
                    <div className="h-1.5 w-12 sm:w-16 bg-zinc-800 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-primary-light rounded-full"
                        style={{ width: `${semanticThreshold * 100}%` }}
                      />
                    </div>
                  </div>
                  <Button 
              ...
              <div className="mt-3 text-sm text-zinc-400">
                <p className="truncate">
                  Showing bills semantically related to: 
                  <span className="text-primary-light">{semanticQuery}</span>
                  {cachedResultsUsed && !loading && (
                    <span className="text-xs text-emerald-400 ml-2">(results from cache)</span>
                  )}
                </p>
              </div>
            </div>
          ...
          {/* Empty state for AI search with no query */}
          {isSemanticSearch && !semanticQuery && (
            <div className="glass-panel p-3 sm:p-6 mb-4 sm:mb-8">
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <Bot className="h-12 w-12 text-primary-light mb-4" />
                <h3 className="text-lg font-medium text-white mb-2">AI Search Mode Activated</h3>
                <p className="text-sm text-zinc-400 max-w-md">
                  Enter your search above and click the AI Search button to find bills based on meaning rather than just keywords.
                </p>
                <div className="mt-4 p-3 bg-zinc-800/50 rounded-lg text-xs text-zinc-300 flex items-center">
                  <span className="text-primary-light mr-2">Pro Tip:</span>
                  Try descriptive queries like "climate change policies" or "healthcare reform proposals"
                </div>
              </div>
            </div>
          )}

          {/* Semantic Bill Graph - only show when semantic search is active, we have a query, and we have results */}
          {isSemanticSearch && semanticQuery && bills.length > 0 && (
            <>
              {process.env.NODE_ENV === 'development' && (
                <div className="glass-panel p-3 sm:p-6 mb-4 text-xs text-zinc-400">
                  <h4 className="font-medium text-zinc-300 mb-2">Debug Info:</h4>
                  <p>Total bills with similarity: {bills.filter(b => b.similarity !== undefined).length}</p>
                  <p>Bills with similarity &gt; {semanticThreshold}: {bills.filter(b => (b.similarity || 0) > semanticThreshold).length}</p>
                  <p>Highest similarity: {Math.max(...bills.map(b => b.similarity || 0)).toFixed(3)}</p>
                  <p>Cache status: {Object.keys(semanticSearchCacheRef.current).length} cached queries</p>
                  <p>Current query cached: {semanticSearchCacheRef.current[semanticQuery.trim().toLowerCase() + '_' + semanticThreshold] ? 'Yes' : 'No'}</p>
                  
                  {/* Show cache details */}
                  {Object.keys(semanticSearchCacheRef.current).length > 0 && (
                    <div className="mt-2">
                      <p className="font-medium text-zinc-300">Cache Details:</p>
                      <div className="mt-1 bg-zinc-800/50 p-2 rounded max-h-24 overflow-y-auto">
                        {Object.entries(semanticSearchCacheRef.current).map(([key, value]) => {
                          const [query, threshold] = key.split('_');
                          const age = Math.round((Date.now() - value.timestamp) / 1000);
                          return (
                            <div key={key} className="flex justify-between text-[10px] py-1 border-b border-zinc-700/50">
                              <span className="text-primary-light/90">{query}</span>
                              <span className="text-zinc-500">{threshold} | {value.bills.length} bills | {age}s ago</span>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <div className="mt-2 mb-1">Similarity values:</div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {bills.map(bill => (
                      <div key={bill.id} className="bg-zinc-800 p-1 rounded flex justify-between">
                        <span>{formatBillType(bill.bill_type)}{bill.bill_number}</span>
                        <span className="text-primary-light">{bill.similarity ? (bill.similarity * 100).toFixed(0) + '%' : 'N/A'}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="glass-panel p-3 sm:p-6">
            <div className="flex flex-col gap-4 mb-4 sm:mb-6">
              <div>
                ...
                <div key={renderKey} className="text-xs sm:text-sm text-zinc-400">
                  {displayedTotalBills} bills analyzed
                </div>
              </div>
            </div>
