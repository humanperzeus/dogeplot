...
export async function analyzeBill(billId: string, billText: string): Promise<void> {
  try {
    ...
    const requestBody = {
      ...
      messages: messages,
      max_[REMOVED]: model.max_[REMOVED],
      temperature: 0.7,
      stream: false
    };
    ...
    console.log('Making OpenRouter API request:', {
      ...
      max[REMOVED]: model.max_[REMOVED]
    });
    ...
    // Calculate cost
    const [REMOVED] = result.usage.total_[REMOVED];
    const cost = ([REMOVED] / 1000) * model.cost_per_1k_[REMOVED];
